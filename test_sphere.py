...
uv_img = torch.cat([img[:, :, :2], torch.zeros(
    480, 640, 1).to(pyredner.get_device())], dim=2)
normal_img = img[:, :, 2:]
pyredner.imwrite(uv_img.cpu(), 'results/test_sphere/uv.png')
pyredner.imwrite(normal_img.cpu(), 'results/test_sphere/normal.png')
