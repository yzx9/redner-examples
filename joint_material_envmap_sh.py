...
mat_grey = pyredner.Material(
    diffuse_reflectance=torch.tensor(
        [0.4, 0.4, 0.4], device=pyredner.get_device()),
    specular_reflectance=torch.tensor(
        [0.5, 0.5, 0.5], device=pyredner.get_device()),
    roughness=torch.tensor([0.02], device=pyredner.get_device()))
materials = [mat_grey]
...
def deringing(coeffs, window):
    ...
    deringed_coeffs[:, 1:1 + 3] += \
        coeffs[:, 1:1 + 3] * \
        math.pow(math.sin(math.pi * 1.0 / window) /
                 (math.pi * 1.0 / window), 4.0)
    deringed_coeffs[:, 4:4 + 5] += \
        coeffs[:, 4:4 + 5] * \
        math.pow(math.sin(math.pi * 2.0 / window) /
                 (math.pi * 2.0 / window), 4.0)
    deringed_coeffs[:, 9:9 + 7] += \
        coeffs[:, 9:9 + 7] * \
        math.pow(math.sin(math.pi * 3.0 / window) /
                 (math.pi * 3.0 / window), 4.0)
...
pyredner.imwrite(
    envmap.cpu(), 'results/joint_material_envmap_sh/target_envmap.exr')
...
envmap = pyredner.SH_reconstruct(deringed_coeffs, res)
pyredner.imwrite(
    envmap.cpu(), 'results/joint_material_envmap_sh/init_envmap.exr')
envmap = pyredner.EnvironmentMap(envmap)
...
diffuse_reflectance_param = \
    torch.tensor([0.3, 0.3, 0.3], device=pyredner.get_device(),
                 requires_grad=True)
specular_reflectance_param = \
    torch.tensor([0.3, 0.3, 0.3], device=pyredner.get_device(),
                 requires_grad=True)
roughness_param = torch.tensor(
    [0.3], device=pyredner.get_device(), requires_grad=True)
...
for t in range(400):
    ...
    envmap = pyredner.SH_reconstruct(deringed_coeffs, res)
    pyredner.imwrite(
        envmap.cpu(), 'results/joint_material_envmap_sh/envmap_{}.exr'.format(t))
    envmap = pyredner.EnvironmentMap(envmap)
    diffuse_reflectance = diffuse_reflectance_param.abs()
    specular_reflectance = specular_reflectance_param.abs()
    ...
    pyredner.imwrite(
        img.cpu(), 'results/joint_material_envmap_sh/iter_{}.png'.format(t))
...
pyredner.imwrite(torch.abs(target - img).cpu(),
                 'results/joint_material_envmap_sh/final_diff.png')
