...
diffuse = (diffuse - np.min(diffuse) + 1e-3) / \
    (np.max(diffuse) - np.min(diffuse))
diffuse = torch.from_numpy(
    np.tile(np.reshape(diffuse, (256, 256, 1)), (1, 1, 3)))
specular = perlin(x, y, seed=1)
specular = (specular - np.min(specular) + 1e-3) / \
    (np.max(specular) - np.min(specular))
specular = torch.from_numpy(
    np.tile(np.reshape(specular, (256, 256, 1)), (1, 1, 3)))
roughness = perlin(x, y, seed=2)
roughness = (roughness - np.min(roughness) + 1e-3) / \
    (np.max(roughness) - np.min(roughness))
...
optimizer = torch.optim.Adam(
    [diffuse_tex, specular_tex, roughness_tex], lr=1e-2)
for t in range(200):
    print('iteration:', t)
    optimizer.zero_grad()
...
pyredner.imwrite(torch.abs(target - img).cpu(),
                 'results/test_svbrdf/final_diff.png')
