...
scene.camera.look_at = torch.tensor(
    [-0.556408, 0.951295, -3.98066], requires_grad=True)
scene.camera.position = torch.tensor(
    [0.00419251, 0.973707, -4.80844], requires_grad=True)
scene.camera.up = torch.tensor(
    [-0.00920347, 0.999741, 0.020835], requires_grad=True)
...
for t in range(600):
    ...
    pyredner.imwrite(
        img.cpu(), 'results/test_living_room/iter_{}.png'.format(t))
